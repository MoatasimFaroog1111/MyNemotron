...
def test_multiple_reviewable_candidates_are_ambiguous_not_unmatched() -> None:
    # Same amount/currency/date => 0.75 each, but no corroborating reference/partner.
    ledger = (
...
def test_benchmark_without_failure_cases_does_not_claim_perfect_recovery() -> None:
    report = BenchmarkReport(
        ...
        results=(
            CaseResult(
                case_id="ordinary",
                task_class=TaskClass.ARABIC_ACCOUNTING,
                passed=True,
                score=1.0,
                latency_ms=10,
                cost_usd=0.01,
...
def test_production_browser_is_blocked_until_capability_gate_is_approved(tmp_path) -> None:
    runtime = build_production_runtime(
        _config(tmp_path, browser_allowed_hosts=("example.com",))
    )
